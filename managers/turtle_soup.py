...
class TurtleSoupManager:
    ...
    def build_judge_system_prompt(puzzle: Dict) -> str:
        ...
        return (
            ...
            f"汤底（绝对保密）：{answer}\n\n"
            "裁判规则：\n"
            "- 当玩家的消息是在针对海龟汤提问时，你只能回答：\n"
            "  「是」「不是」「不相关」「不完全是」或「🎉 正确！」\n"
            "- 可以在回答后加一小句引导（不能透露关键信息）\n"
            "- 绝对不能主动透露汤底任何细节\n"
            "- 如果玩家基本猜出完整真相，回答「🎉 正确！」并简要复述\n"
            "- 如果玩家在聊与海龟汤无关的话题，正常回复即可\n"
            "</海龟汤游戏>"
        )

    @staticmethod
    def build_judge_direct_prompt(answer: str, question: str) -> str:
        """构建 session_waiter 内直接调用 LLM 的完整 prompt"""
        return (
            "你是海龟汤游戏的裁判。\n"
            f"【汤底（绝对保密）】：{answer}\n\n"
            "规则：只能回答「是」「不是」「不相关」「不完全是」「🎉 正确！」\n"
            "可以加一小句引导提示，但绝不能透露汤底细节。\n"
            "如果玩家基本猜出完整真相，回答「🎉 正确！」并简要复述汤底。\n\n"
            f"玩家提问：{question}"
        )
